...
export default function App() {
  ...
  const images = animalData.map((photo) => photo.image)

  const handleForwardClick = () => {
    setData((pre) => (pre === images.length - 1 ? 0 : pre + 1))
  }

  const handleBackClick = () => {
    setData((pre) => (pre === 0 ? images.length - 1 : pre - 1))
  }

  return (
    <div className="container">
      <header>
        <h3 className="text">Project 1: Carousel </h3>
      </header>
      <div className="carousel">
        <button className="btn btn-outline-secondary" onClick={handleBackClick}>
          &#8249; Pre
        </button>
        <img src={images[data]} alt="carousel" />
        <button
